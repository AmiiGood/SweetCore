...
export const EnsambleCard = ({ ensamble, index }) => {
  ...
  const itemQuantity = getItemQuantity(`ensamble-${ensamble.id}`);
  ...
  const handleAddToCart = async () => {
    ...
    addItem({
      id: `ensamble-${ensamble.id}`,
      name: ensamble.name,
      price: ensamble.price,
    ...
    showSuccess(
      ...
      `${ensamble.name} ha sido agregada al carrito`,
      {
        type: "cart",
        action: {
          label: "Ver carrito",
  ...
  const handleQuantityChange = (newQuantity) => {
    ...
    if (newQuantity <= 0) {
      updateQuantity(`ensamble-${ensamble.id}`, 0);
      showInfo(
        "PC eliminada",
        `${ensamble.name} ha sido eliminada del carrito`
      );
    } else {
      updateQuantity(`ensamble-${ensamble.id}`, newQuantity);

      if (newQuantity > oldQuantity) {
        showSuccess(
          "Cantidad actualizada",
          `Ahora tienes ${newQuantity} unidades de ${ensamble.name}`
        );
      }
    }
  };
  ...
  const handleToggleFavorite = () => {
    ...
    if (!isFavorite) {
      showSuccess(
        ...
        `${ensamble.name} ha sido agregado a tus favoritos`,
        {
          duration: 3000,
          action: {
            label: "Ver favoritos",
      ...
      showInfo(
        ...
        `${ensamble.name} ha sido eliminado de tus favoritos`
      );
    }
  };
  ...
  const specs = {
    ...
    ram: ensamble.specs.ram || "16GB DDR4",
    storage: ensamble.specs.storage || "1TB SSD",
  };
          ...
          >
            <div className="bg-white/90 backdrop-blur-sm rounded-lg p-3">
              <div className="flex items-center justify-between text-xs font-medium text-gray-700 mb-2">
                ...
                <span>{ensamble.performance}%</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-600 mb-3">
              {Object.entries(specs).map(([key, value], i) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1 font-medium capitalize text-gray-800 mb-1">
                      <SpecIcon size={10} />
                      {key === "ram" ? "RAM" : key.toUpperCase()}
                    </div>
